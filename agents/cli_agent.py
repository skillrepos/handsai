...
def grep_code(pattern):
    """Search the inventory service's source for a pattern using grep."""
    # TODO: Lab 2 merge step — call run_command with the right grep invocation.
    raise NotImplementedError("merge in the completed code (see labs.md)")


def run_tests():
    """Run the inventory service's test suite with pytest."""
...
TOOLS = {
    "grep_code": {
        "function": grep_code,
        "description": "Search the inventory service source code for a pattern (runs grep).",
        "args": {"pattern": "string, text or regex to search for"},
    },
    "run_tests": {
        "function": run_tests,
        "description": "Run the inventory service test suite (runs pytest).",
        "args": {},
    },
    "git_history": {
