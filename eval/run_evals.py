...
Agents are non-deterministic, so "it worked when I tried it" is not a
test strategy. This harness runs a set of scenarios through the safe
agent and applies simple, deterministic checks to each run:
...
def run_check(check, final, tool_calls, tickets_added):
    """Apply one deterministic check. Returns (passed, description)."""
